...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('LOGISTICS create project error:', error);
...
router.get('/', async (req, res) => {
  try {
    const projects = await req.models.LogisticsProject.findAll({
    ...
    console.error('LOGISTICS list projects error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const project = await req.models.LogisticsProject.findOne({
      where,
      include: [
        { model: req.models.LogisticsUploadedFile, as: 'files' },
    ...
    console.error('LOGISTICS get project error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.patch('/:id', async (req, res) => {
  try {
    const project = await req.models.LogisticsProject.findByPk(req.params.id);
    if (!project) {
      return res.status(404).json({ success: false, error: 'Project not found' });
    }
    ...
    console.error('LOGISTICS update project error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.post('/:projectId/api-key', async (req, res) => {
  try {
    ...
    const existingKey = await req.models.LogisticsApiKey.findOne({
    ...
    const apiKeyRecord = await req.models.LogisticsApiKey.create({
    ...
    console.error('LOGISTICS API key generate error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.get('/:projectId/api-key', async (req, res) => {
  try {
    const key = await req.models.LogisticsApiKey.findOne({
    ...
    console.error('LOGISTICS API key status error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.delete('/:projectId/api-key', async (req, res) => {
  try {
    const updated = await req.models.LogisticsApiKey.update(
      { is_active: false },
      { where: { project_id: req.params.projectId, is_active: true } }
    );
    ...
    console.error('LOGISTICS API key revoke error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
