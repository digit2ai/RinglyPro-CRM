...
router.post('/generate', async (req, res) => {
  try {
    ...
    const project = await req.models.LogisticsProject.create({
      ...
      contact_email: 'demo@logistics.com',
    ...
    await req.models.LogisticsItemMaster.bulkCreate(itemRecords);
    ...
    await req.models.LogisticsInventoryData.bulkCreate(invRecords);
    ...
    await req.models.LogisticsGoodsInData.bulkCreate(giRecords);
    ...
    for (let i = 0; i < goRecords.length; i += BATCH_SIZE) {
      await req.models.LogisticsGoodsOutData.bulkCreate(goRecords.slice(i, i + BATCH_SIZE));
    }
    ...
    const analysisMap = {};
    const savedResults = await req.models.LogisticsAnalysisResult.findAll({
    ...
    await req.models.LogisticsProductRecommendation.bulkCreate(recRecords);
    ...
    res.status(201).json({
      ...
      data: {
        ...
        dashboard_url: `/logistics/?project=${project.project_code}`
      }
    });
  } catch (error) {
    console.error('LOGISTICS demo generate error:', error);
