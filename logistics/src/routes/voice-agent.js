...
// ============================================================================
// LOGISTICS Voice Agent Endpoints
...
function buildSpokenBriefing(project, analysisMap, recommendations, benefits) {
  ...
  if (recommendations && recommendations.length > 0) {
    parts.push(`Based on the analysis, ${recommendations.length} RinglyPro Logistics products are recommended.`);
...
router.get('/:projectId/full-briefing', async (req, res) => {
  try {
    ...
    console.error('LOGISTICS voice briefing error:', error);
...
router.get('/:projectId/overview', async (req, res) => {
  try {
    ...
    const recCount = await req.models.LogisticsProductRecommendation.count({
      where: { project_id: project.id }
    });
...
router.get('/:projectId/order-analysis', async (req, res) => {
  try {
    ...
    const [orderResult, throughputResults] = await Promise.all([
      req.models.LogisticsAnalysisResult.findOne({
        where: { project_id: project.id, analysis_type: 'order_structure' }
      }),
      req.models.LogisticsAnalysisResult.findAll({
...
router.get('/:projectId/product-recommendations', async (req, res) => {
  try {
    ...
    const fitResult = await req.models.LogisticsAnalysisResult.findOne({
  ...
  try {
    ...
    try {
      const [tables] = await seq.query(
        "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'logistics_telemetry_events'"
      );
