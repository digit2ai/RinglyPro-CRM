...
async function storeResult(models, projectId, analysisType, data) {
  await models.LogisticsAnalysisResult.upsert({
...
async function computeOverviewKPIs(models, projectId) {
  ...
  const totalSKUs = await models.LogisticsItemMaster.count({ where: { project_id: projectId } });
  const binCapableSKUs = await models.LogisticsItemMaster.count({
...
async function computeFitAnalysis(models, projectId) {
  ...
  const items = await models.LogisticsItemMaster.findAll({
    where: { project_id: projectId },
