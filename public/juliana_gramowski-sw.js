...
const CORE = [
  ...
  '/juliana_gramowski-192.png',
  '/juliana_gramowski-512.png',
  '/juliana_gramowski-apple-touch.png'
];
  ...
  if (req.mode === 'navigate') {
    e.respondWith(
      fetch(req).then(function (r) {
        var cp = r.clone();
        caches.open(CACHE).then(function (c) { c.put('/juliana_gramowski', cp); });
        return r;
      }).catch(function () {
        return caches.match('/juliana_gramowski').then(function (m) { return m || caches.match(req); });
      })
    );
    return;
  }
