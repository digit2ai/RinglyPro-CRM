...
import { Lock, ChevronLeft, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function SponsorResetPasswordPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center px-4">
        <div className="w-full max-w-md text-center">
          ...
          <Link
            to="/sponsor/login"
            className="inline-block px-6 py-3 bg-amber-500 text-black font-semibold rounded-lg hover:bg-amber-600 transition-colors"
          >
            Back to Sign In
          </Link>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/tunjoracing/api/v1/sponsors/reset-password-token', {
        method: 'POST',
