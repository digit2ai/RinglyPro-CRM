...
import { Mail, ChevronLeft, CheckCircle, Key, Copy } from 'lucide-react';
...
export default function SponsorForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/tunjoracing/api/v1/sponsors/forgot-password', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <Link to="/sponsor/login" className="flex items-center gap-1 text-slate-400 hover:text-white mb-8 transition-colors">
        ...
        <div className="bg-slate-800/50 rounded-lg p-8 border border-slate-700">
          {resetLink ? (
            <div className="text-center">
              ...
              <p className="text-amber-400 font-semibold mb-5">{email}</p>
              ...
              <div className="space-y-3">
                <a
                  href={resetLink}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-amber-500 text-black font-semibold rounded-lg hover:bg-amber-600 transition-colors"
                >
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3 bg-amber-500 text-black font-semibold rounded-lg hover:bg-amber-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  <Key className="h-4 w-4" />
