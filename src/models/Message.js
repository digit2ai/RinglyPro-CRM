...
Message.findByClient = function(clientId, options = {}) {
  return this.findAll({
    where: { clientId },
    attributes: ['id', 'clientId', 'contactId', 'twilioSid', 'recordingUrl', 'direction', 'fromNumber', 'toNumber', 'body', 'status', 'read', 'messageSource', 'messageType', 'callDuration', 'callStartTime', 'createdAt', 'updatedAt'],
    order: [['createdAt', options.order || 'DESC']],
    limit: options.limit || 50,
    offset: options.offset || 0
  });
