...
/**
 * API Key Authentication Middleware for PINAXIS Production Ingest API
 *
...
const requireApiKey = async (req, res, next) => {
  try {
    ...
    const PinaxisApiKey = req.models.PinaxisApiKey;
    if (!PinaxisApiKey) {
      return res.status(500).json({ success: false, error: 'API key service not available' });
    }

    const candidates = await PinaxisApiKey.findAll({
    ...
    console.error('PINAXIS API auth error:', error);
