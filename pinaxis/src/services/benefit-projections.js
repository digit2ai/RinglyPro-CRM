...
/**
 * PINAXIS Client Benefit Projections Engine
 *
...
function computeDowntimeReduction(m, topScore) {
  ...
  return {
    ...
    data_drivers: [
      ...
      { metric: 'top_product_score', value: Math.round(topScore), explanation: `Best-fit GEBHARDT system scores ${Math.round(topScore)}/100 for this profile` }
    ],
...
function computeSparePartsWaste(m, numProducts) {
  ...
  return {
    ...
    reasoning: `Managing ${m.total_skus} SKUs (${round1((1 - m.active_sku_ratio) * 100)}% dormant) across ${numProducts} GEBHARDT systems — predictive ordering reduces spare parts waste by ~${round1(pct)}%.`
  };
}
...
function computeSalesCycle(m) {
  return {
    ...
    reasoning: `PINAXIS automated analysis of ${m.total_orders.toLocaleString()} orders and ${m.total_skus} SKUs replaces weeks of manual data gathering with instant, data-driven GEBHARDT proposals — reducing sales cycles by ~30%.`
  };
}
...
function computeTechnicianAccuracy(m, numProducts) {
  return {
    ...
    confidence: 'medium',
    data_drivers: [
      { metric: 'products_matched', value: numProducts, explanation: `${numProducts} distinct GEBHARDT systems require specialized service knowledge` },
      { metric: 'total_skus', value: m.total_skus, explanation: `${m.total_skus} SKU profiles enable precise fault diagnosis` }
    ],
    reasoning: `With ${numProducts} GEBHARDT product systems handling ${m.total_skus} SKUs, AI-driven diagnostics achieve 92%+ first-time resolution by matching issue patterns to the right technician expertise.`
  };
}
