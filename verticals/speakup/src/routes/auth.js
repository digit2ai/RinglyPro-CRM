...
/**
 * SpeakUp — authentication (cookie-based JWT). LOGIN ONLY — no public signup.
 * Accounts are seeded by the owner (services/users.js). Each user is their own
 * private tenant (tenant_id = user id) so recordings are isolated per person.
 *
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.error('SpeakUp login error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

router.post('/logout', (req, res) => {
  res.clearCookie(COOKIE, { path: '/speakup' });
