...
router.post('/research', mutation, operator, wrap(async (req, res) => {
  ...
  try {
    let remembered = '';
    try { remembered = await memory.contextBlock(tenantOf(req), { project_key: b.project_key ? String(b.project_key).slice(0, 60) : null }); } catch (e) {}
...
router.get('/rules', operator, wrap(async (req, res) => {
  const pk = req.query.project ? String(req.query.project).slice(0, 60) : null;
  const r = await memory.rules(tenantOf(req), pk);
  res.json({ rules: r.text, project_key: pk, is_default: r.is_default, max: memory.RULES_MAX, default_rules: memory.DEFAULT_RULES });
}));
router.put('/rules', mutation, operator, wrap(async (req, res) => {
  ...
  const r = await memory.setRules(tenantOf(req), (req.body || {}).rules, pk);
  ...
  res.json({ rules: r.text, project_key: r.project_key, is_default: false });
}));
// What it remembers right now, so the owner can see it rather than trust it.
router.get('/memory', operator, wrap(async (req, res) => {
  const pk = req.query.project ? String(req.query.project).slice(0, 60) : null;
  res.json({ project_key: pk, recent_work: await memory.recentWork(tenantOf(req), { project_key: pk }),
