...
(function () {
  ...
  // ---------- icons ----------
  var DIAMOND = '<svg viewBox="0 0 24 24" width="17" height="17" style="vertical-align:-3px"><path d="M12 2 L22 12 L12 22 L2 12 Z" fill="none" stroke="#14201b" stroke-width="1.6"/><path d="M9 14 L12 8 L15 14" fill="none" stroke="#14201b" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  var HB = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#5c6b64" stroke-width="2"><line x1="4" y1="7" x2="20" y2="7"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="17" x2="20" y2="17"/></svg>';
  var SUN = '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="#5c6b64" stroke-width="1.8"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2M5 5l1.5 1.5M17.5 17.5L19 19M19 5l-1.5 1.5M6.5 17.5L5 19" stroke-linecap="round"/></svg>';
  ...
  var ARROW = '<svg viewBox="0 0 24 24" width="30" height="30" fill="none" stroke="#14201b" stroke-width="1.8"><rect x="3" y="3" width="18" height="18" rx="6"/><path d="M12 8l4 6H8z" fill="#14201b" stroke="none"/></svg>';
  ...
  function ring(sz) { sz = sz || 60; var r = sz / 2 - 5, c = 2 * Math.PI * r, off = c * (1 - SCORE / 100);
    return '<svg viewBox="0 0 ' + sz + ' ' + sz + '" width="' + sz + '" height="' + sz + '"><circle cx="' + sz / 2 + '" cy="' + sz / 2 + '" r="' + r + '" fill="none" stroke="#e3ede8" stroke-width="5"/><circle cx="' + sz / 2 + '" cy="' + sz / 2 + '" r="' + r + '" fill="none" stroke="#35a768" stroke-width="5" stroke-linecap="round" stroke-dasharray="' + c.toFixed(1) + '" stroke-dashoffset="' + off.toFixed(1) + '" transform="rotate(-90 ' + sz / 2 + ' ' + sz / 2 + ')"/><text x="' + sz / 2 + '" y="' + (sz / 2 + 6) + '" text-anchor="middle" fill="#14201b" font-size="' + (sz * 0.3) + '" font-weight="800" font-family="Inter,sans-serif">' + SCORE + '</text></svg>'; }
  ...
  var SCREENS = {
    ...
    patrimonio: function () {
      return header() + '<div class="psim-scroll">' + titleRow('Mi Patrimonio', 'Todas tus finanzas en una sola lectura.') +
        ...
        '<div class="psim-bar-row"><span>Activos</span><b>' + money(64000000) + '</b></div><div class="psim-bar"><i style="width:100%;background:#2f9e63"></i></div>' +
        ...
        '<div class="psim-bar-row" style="border-top:1px solid rgba(0,0,0,.08);padding-top:8px"><span>Patrimonio neto</span><b>' + money(43000000) + '</b></div>' +
        '</div></div>' + maya();
    },
    metas: function () {
      function goal(name, tag, cur, tgt, pct, meses) {
        return '<div class="psim-goal"><div class="psim-goal-hd"><div class="psim-goal-ic">◎</div><div class="psim-goal-x"><b>' + name + '</b><small>' + tag + '</small></div><div class="psim-goal-a"><b>' + money(cur) + '</b><small>de ' + money(tgt) + '</small></div></div>' +
          '<div class="psim-bar"><i style="width:' + pct + '%;background:#2f9e63"></i></div><div class="psim-goal-ft"><b>' + pct + '%</b> completado<span>' + meses + ' meses restantes</span></div></div>';
      }
    ...
    diagnostico: function () {
      function pill(name, w, sc) { var col = sc >= 70 ? '#2f9e63' : sc >= 45 ? '#e0b34a' : '#e08a6a'; return '<div class="psim-pilar"><div class="psim-pilar-t"><b>' + name + '</b> · ' + w + '%<span style="color:' + col + '">' + sc + '</span></div><div class="psim-bar"><i style="width:' + sc + '%;background:' + col + '"></i></div></div>'; }
    ...
    salud: function () {
      function area(name, sc) { var col = sc >= 70 ? '#2f9e63' : sc >= 45 ? '#e0b34a' : '#e08a6a'; return '<div class="psim-area"><span class="psim-area-dot" style="background:' + col + '"></span><span class="psim-area-n">' + name + '</span><b class="psim-area-s" style="color:' + col + '">' + sc + '</b></div>'; }
  ...
  var CSS = '' +
    ...
    '.psim-phone{width:300px;max-width:84vw;height:600px;max-height:76vh;margin:0 auto;background:#0c100e;border-radius:46px;padding:12px;box-shadow:0 34px 90px rgba(0,0,0,.5),0 0 0 2px #05080a;position:relative}' +
    '.psim-notch{position:absolute;top:16px;left:50%;transform:translateX(-50%);width:96px;height:22px;background:#0c100e;border-radius:0 0 14px 14px;z-index:5}' +
    '.psim-screen{height:100%;background:#f4f8f6;color:#14201b;border-radius:34px;overflow:hidden;display:flex;flex-direction:column;position:relative}' +
    ...
    '.psim-ic{width:30px;height:30px;border-radius:9px;border:1px solid rgba(0,0,0,.08);display:flex;align-items:center;justify-content:center}' +
    ...
    '.psim-sub{color:#5c6b64;font-size:11.5px;line-height:1.4;margin:3px 0 0}' +
    '.psim-scb{flex:0 0 auto;text-align:center}.psim-scl{font-size:7.5px;letter-spacing:.12em;color:#5c6b64;font-weight:700;margin-top:2px}.psim-scr{font-size:12px;font-weight:800;font-style:italic;color:#2f9e63}' +
    '.psim-cap{font-size:9px;letter-spacing:.11em;color:#6b7c74;font-weight:700;margin-top:16px}' +
    '.psim-cap2{font-size:8.5px;letter-spacing:.11em;color:#5c6b64;font-weight:700;display:block}' +
    '.psim-big{font-family:"Source Sans 3",sans-serif;font-weight:800;font-size:30px;letter-spacing:-.02em;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.psim-big span{font-size:12px;color:#5c6b64;font-weight:600}' +
    '.psim-hr{height:1px;background:rgba(0,0,0,.08);margin:14px 0 0}' +
    '.psim-summary{margin-top:16px}.psim-sm-hd{background:linear-gradient(120deg,rgba(47,158,99,.16),rgba(126,224,160,.03));border-left:3px solid #2f9e63;border-radius:12px;padding:12px 13px;display:flex;justify-content:space-between;align-items:center}' +
    '.psim-sm-hd b{font-size:13px}.psim-sm-hd small{display:block;color:#5c6b64;font-size:10px;margin-top:1px}.psim-sm-a{font-size:14px}' +
    '.psim-card{margin-top:10px;background:#ffffff;border:1px solid rgba(0,0,0,.08);border-radius:14px;padding:13px 13px 6px;box-shadow:0 4px 16px rgba(15,26,23,.05)}' +
    ...
    '.psim-add{color:#2f9e63;font-size:11.5px;font-weight:700}' +
    '.psim-li{display:flex;align-items:center;gap:8px;padding:9px 0;border-top:1px solid rgba(0,0,0,.06)}' +
    '.psim-li-x{flex:1;min-width:0}.psim-li-n{font-weight:700;font-size:12.5px}.psim-li-s{color:#5c6b64;font-size:10px;margin-top:1px}' +
    '.psim-li-a{font-weight:800;font-size:12.5px;white-space:nowrap}.psim-li-ic{opacity:.55;flex:0 0 auto}' +
    '.psim-herocard{margin-top:8px;border-radius:18px;padding:14px;background:radial-gradient(120% 120% at 15% 0,rgba(47,158,99,.16),transparent 62%),#e9f6ee;border:1px solid rgba(0,0,0,.08)}' +
    ...
    '.psim-chip{margin-top:10px;display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(0,0,0,.12);border-radius:99px;padding:6px 12px;font-size:11px;color:#46554e}.psim-dot{width:7px;height:7px;border-radius:50%;background:#2f9e63}' +
    '.psim-blk{margin-top:16px}.psim-blk-hd{display:flex;justify-content:space-between;align-items:center}.psim-blk-hd b{font-size:16px}.psim-link{color:#2f9e63;font-size:11.5px;font-weight:700}' +
    '.psim-btns{display:flex;gap:8px;margin-top:12px}.psim-btn-light{flex:1;background:#14201b;color:#fff;border:0;border-radius:12px;padding:11px;font-weight:800;font-size:12px}.psim-btn-ghost{flex:1;background:transparent;color:#14201b;border:1px solid rgba(0,0,0,.14);border-radius:12px;padding:11px;font-weight:700;font-size:12px}' +
    '.psim-bar-row{display:flex;justify-content:space-between;align-items:center;font-size:13px;margin-top:8px}.psim-bar{height:7px;border-radius:99px;background:rgba(0,0,0,.08);overflow:hidden;margin-top:5px}.psim-bar i{display:block;height:100%;border-radius:99px}' +
    '.psim-goal{padding:12px 0;border-top:1px solid rgba(0,0,0,.06)}.psim-goal-hd{display:flex;align-items:center;gap:9px}.psim-goal-ic{width:30px;height:30px;border-radius:9px;background:rgba(47,158,99,.16);color:#2f9e63;display:flex;align-items:center;justify-content:center;font-size:15px}.psim-goal-x{flex:1}.psim-goal-x b{font-size:13px}.psim-goal-x small{display:block;color:#5c6b64;font-size:10px}.psim-goal-a{text-align:right}.psim-goal-a b{font-size:13px}.psim-goal-a small{display:block;color:#5c6b64;font-size:9.5px}.psim-goal-ft{display:flex;justify-content:space-between;font-size:10.5px;color:#5c6b64;margin-top:6px}.psim-goal-ft b{color:#14201b}' +
    '.psim-newgoal{margin-top:10px;display:flex;align-items:center;gap:10px;border:1px dashed rgba(0,0,0,.14);border-radius:14px;padding:13px}.psim-ng-plus{width:32px;height:32px;border-radius:50%;background:rgba(47,158,99,.16);color:#2f9e63;display:flex;align-items:center;justify-content:center;font-size:20px}.psim-newgoal b{font-size:13px}.psim-newgoal small{display:block;color:#5c6b64;font-size:10px}' +
    '.psim-diag{margin-top:14px;background:#ffffff;border:1px solid rgba(0,0,0,.08);border-radius:16px;padding:16px 13px;text-align:center;display:flex;flex-direction:column;align-items:center}.psim-diag-lbl{color:#2f9e63;font-size:10px;letter-spacing:.12em;font-weight:800}.psim-diag-hook{font-size:15px;margin:8px 0 10px;line-height:1.3}.psim-diag-ring{margin:2px 0}.psim-diag-chip{border:1px solid #2f9e63;color:#2f9e63;border-radius:99px;padding:5px 16px;font-weight:800;font-size:12px;margin-top:8px}' +
    ...
    '.psim-area{display:flex;align-items:center;gap:9px;padding:9px 0;border-top:1px solid rgba(0,0,0,.06)}.psim-area-dot{width:10px;height:10px;border-radius:50%;flex:0 0 auto}.psim-area-n{flex:1;font-weight:700;font-size:12.5px}.psim-area-s{font-size:13px}' +
    '.psim-login{display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding-top:60px;background:radial-gradient(120% 80% at 50% 0,rgba(47,158,99,.12),transparent 55%)}' +
    '.psim-lg-mark{width:64px;height:64px;border-radius:16px;border:1px solid rgba(0,0,0,.18);display:flex;align-items:center;justify-content:center;margin-bottom:16px}' +
    ...
    '.psim-inp{width:100%;background:#fff;color:#14201b;border:1px solid rgba(0,0,0,.14);border-radius:12px;padding:13px 14px;font-size:13px;text-align:left}.psim-inp-ph{color:#8792a0}' +
    '.psim-grad{width:100%;margin-top:16px;border:0;border-radius:12px;padding:14px;font-weight:800;font-size:14px;color:#fff;background:linear-gradient(90deg,#2f9e63,#2aa39a)}' +
    '.psim-lg-foot{margin-top:16px;font-size:12px;color:#5c6b64}.psim-lg-foot b{color:#2f9e63}' +
    '.psim-maya{position:absolute;left:0;right:0;bottom:0;display:flex;align-items:center;gap:9px;justify-content:flex-end;padding:12px 14px;background:linear-gradient(0deg,#f4f8f6 72%,transparent)}.psim-orb{width:40px;height:40px;border-radius:50%;background:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 18px rgba(15,26,23,.18);border:1px solid rgba(0,0,0,.06)}.psim-maya-t{text-align:right}.psim-maya-t b{font-size:12.5px}.psim-maya-t small{display:block;color:#5c6b64;font-size:10px}' +
    /* controls */
