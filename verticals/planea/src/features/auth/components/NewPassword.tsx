...
export function NewPassword(props: Props) {
  ...
  return (
    <div className="flex min-h-screen flex-col justify-center bg-(--off-white) px-6 py-12 dark:bg-(--primary-400) lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-sm">
        ...
        <h2 className="mt-10 text-center text-2xl/9 font-bold tracking-tight text-(--dark) dark:text-white">
          {t('newPassword.form.title')}
        </h2>
        <p className="mt-2 text-center text-sm text-(--gray-300)">
          {t('newPassword.form.description')}
        </p>
      </div>

      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        {isChecking ? (
          <p className="text-center text-sm text-(--gray-300)">{t('newPassword.form.checkingText')}</p>
        ) : isRecoverySession === false ? (
          <p className="text-center text-sm text-red-500">{t('newPassword.form.invalidLinkText')}</p>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              >
                {t('newPassword.form.passwordLabel')}
              </label>
              <div className="mt-2">
                <input
                  id="password"
            ...
            <div>
              <label
                ...
                className="block text-sm/6 font-medium text-(--dark) dark:text-(--gray-100)"
              >
                {t('newPassword.form.confirmPasswordLabel')}
              </label>
              <div className="mt-2">
                <input
                  id="confirmPassword"
            ...
            <div>
              ...
              >
                {loading ? t('newPassword.form.submittingButton') : t('newPassword.form.submitButton')}
              </button>
            </div>
          </form>
        )}
