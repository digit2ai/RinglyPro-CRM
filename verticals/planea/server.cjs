...
router.post('/api/v1/maya/chat', express.json({ limit: '256kb' }), async (req, res) => {
  try {
    ...
    if (!r.ok) {
      ...
      let emsg = '';
      try { const ej = JSON.parse(t); emsg = (ej && ej.error && (ej.error.message || ej.error.type)) || ''; } catch (e) { emsg = t.slice(0, 200); }
      return res.status(502).json({ error: 'maya_upstream', upstream_status: r.status, upstream_msg: String(emsg).slice(0, 220), used_model: MAYA_MODEL, reply: 'Tuve un problema para responder en este momento. Intenta de nuevo en unos segundos.' });
    }
...
if (hasPortal) {
  ...
  router.get('/forgot', (req, res) => { noCache(res); res.sendFile(path.join(portalDir, 'forgot.html')); });
  router.get('/reset', (req, res) => { noCache(res); res.sendFile(path.join(portalDir, 'reset.html')); });
