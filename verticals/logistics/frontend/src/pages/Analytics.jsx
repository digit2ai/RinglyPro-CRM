...
export default function Analytics() {
  ...
  const loadDashboard = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/analytics/dashboard', { params: { date_from: dateFrom, date_to: dateTo } });
  ...
  const loadCustomers = async () => {
    try {
      const { data } = await api.get('/analytics/customers', { params: { days: 90 } });
  ...
  const loadExceptions = async () => {
    try {
      const { data } = await api.get('/analytics/exceptions', { params: { days: 30 } });
