...
const ENSURE_TABLES = `
  CREATE TABLE IF NOT EXISTS lg_ndas (
  ...
  CREATE TABLE IF NOT EXISTS lg_nda_signatures (
    id SERIAL PRIMARY KEY,
...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('Logistics NDA submit error:', error);
  ...
  try {
    ...
    const [rows] = await sequelize.query(
      `SELECT id, project_id, disclosing_company, disclosing_name, disclosing_title,
       ...
       FROM lg_ndas ORDER BY created_at DESC`
    );
    res.json({ success: true, data: rows });
  } catch (error) {
    console.error('Logistics NDA list error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await sequelize.query(
      `SELECT * FROM lg_ndas WHERE id = $1`, { bind: [req.params.id] }
    );
...
router.post('/signatures', async (req, res) => {
  try {
    ...
    const [result] = await sequelize.query(`
      INSERT INTO lg_nda_signatures (signer_role, company, name, title, signature_data, signed_at)
      VALUES ($1, $2, $3, $4, $5, $6)
    ...
    console.error('Logistics NDA signature save error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const [rows] = await sequelize.query(
      `DELETE FROM lg_ndas WHERE id = $1 RETURNING id`, { bind: [req.params.id] }
    );
    if (!rows.length) return res.status(404).json({ success: false, error: 'NDA not found' });
