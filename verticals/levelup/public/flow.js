/* LevelUp workflow circuit (adapted from BuyersLine). The page ships with every step readable (the
...
(function () {
  ...
  function build() {
    ...
    var defs = el('defs', {});
    var g = el('linearGradient', { id: 'luFlowGrad', gradientUnits: 'userSpaceOnUse', x1: pts[0].x, y1: 0, x2: pts[pts.length - 1].x, y2: 0 });
    ...
    progress = el('path', { d: d, 'class': 'c-progress', stroke: 'url(#luFlowGrad)' });
    tail = el('path', { d: d, 'class': 'c-tail', stroke: 'url(#luFlowGrad)' });
