...
(function () {
  ...
  function viewSettings() {
    Promise.all([api('/api/v1/keys'), api('/api/v1/audit')]).then(function (r) {
      ...
      view.innerHTML = '<div class="panel"><h2>' + L('More', 'Más') + '</h2><div class="acts">' + ['strategy', 'ideas', 'editing', 'business', 'picks', 'research'].map(function (k) { return '<a class="btn small" href="#' + k + '">' + t(k) + '</a>'; }).join('') + '<a class="btn small" href="' + B + '/requirements">' + L('Requirements', 'Requisitos') + '</a></div></div>' +
