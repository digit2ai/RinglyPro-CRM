    ...
    html: `
      ...
      ${cb?.conversion_formula ? `
        <div style="background:rgba(14,165,233,0.10);border:2px solid #0ea5e9;border-radius:12px;padding:18px;margin:14px 0">
          <div style="font-size:11px;text-transform:uppercase;letter-spacing:2px;color:#7dd3fc;font-weight:700;margin-bottom:6px">The Conversion Formula — every number flows from this</div>
          <div style="font-size:18px;color:#f8fafc;font-weight:700;margin-bottom:6px">Incremental = ${cb.conversion_formula.basis_pct}% × Open Soft-Tissue cases the da Vinci can perform</div>
          <div style="font-size:14px;color:#cbd5e1;margin-bottom:10px">For ${esc(hospitalName)}: ${cb.conversion_formula.basis_pct}% × ${fmt(cb.conversion_formula.open_applicable_cases)} open applicable cases = <strong style="color:#34d399">${fmt(cb.conversion_formula.incremental_opportunity)} incremental cases</strong>. Laparoscopic and robotic cases — and any open case outside these procedures — do <strong style="color:#fca5a5">NOT count</strong>.</div>
