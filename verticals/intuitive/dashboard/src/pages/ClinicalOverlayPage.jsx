    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      {enrichment?.conversion_formula && (
        <div className="bg-sky-900/15 border-2 border-sky-600/50 rounded-xl p-5 mb-6">
          <div className="text-[10px] uppercase tracking-widest text-sky-300 font-bold mb-1">The Conversion Formula — the cost-avoidance numbers flow from this</div>
          <div className="text-lg font-bold text-white mb-1">Conversion opportunity = {enrichment.conversion_formula.basis_pct}% × Open Soft-Tissue cases the da Vinci can perform</div>
          <div className="text-sm text-slate-300 mb-3">For {enrichment.hospital_name}: {enrichment.conversion_formula.basis_pct}% × {fmt(enrichment.conversion_formula.open_applicable_cases)} open applicable cases = <span className="text-emerald-300 font-bold">{fmt(enrichment.conversion_formula.conversion_opportunity || enrichment.conversion_formula.incremental_opportunity)} converted cases</span> (cost avoidance — not the surgeon-committed incremental volume from Step 7). Laparoscopic and robotic cases — and any open case outside these procedures — do <span className="text-red-300 font-semibold">NOT count</span>.</div>
