...
export default function BrokerageDemo() {
  ...
  const loadWorkspaces = async () => {
    try {
      const { data } = await api.get('/brokerage-demo/workspaces');
  ...
  const createWorkspace = async () => {
    setLoading(true);
    try {
      const { data } = await api.post('/brokerage-demo/workspace', form);
  ...
  const generateData = async (code) => {
    try {
      await api.post('/brokerage-demo/generate', { access_code: code, sample_size: 50 });
      loadWorkspaces();
    } catch (err) { console.error(err); }
  };
