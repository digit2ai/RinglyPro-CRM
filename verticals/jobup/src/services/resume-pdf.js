...
/**
 * Résumé PDF rendering, server-side, via pdfkit.
 *
 * PORTED VERBATIM from verticals/citijobs/src/services/pdf.js, whose layout is
 * already proven against real bank ATS submissions. The two are deliberately
 * separate files rather than a shared module: citijobs is a private, single-
 * owner tool on its own release cadence, and coupling a paid subscriber feature
 * to it would mean a tweak there could silently change what a customer sends to
 * an employer.
 *
 ...
 * redeploy and take the 'recover the exact document I sent them' guarantee with
...
function filename(profileName, employer, version) {
  const clean = (x, fb) => String(x || fb).replace(/[^A-Za-z0-9]+/g, '_').replace(/^_|_$/g, '');
  const who = clean(profileName, 'Resume');
  const where = employer ? '_' + clean(employer, '') : '';
  const v = Number(version || 1) > 1 ? `_v${version}` : '';
  return `${who}_Resume${where}${v}.pdf`;
}
