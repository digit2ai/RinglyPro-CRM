...
/**
 * Send a PREVIEW of the job-match digest for a tenant to any address, without
 * touching state: nothing is stamped, no email_sends row, no cap advance. Uses
 * the exact production render path (SendGrid template when configured, else the
 * in-app fallback). Operator-only.
 *   POST /admin/subscribers/:tenantId/notify-preview?to=<email>
 */
router.post('/subscribers/:tenantId/notify-preview', requireOwner, async (req, res) => {
  const tenantId = parseInt(req.params.tenantId, 10);
  ...
  try {
    ...
    await notifier.ensureUnsubToken(sub);                       // so the footer link is real
