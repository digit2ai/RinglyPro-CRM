...
New-construction buyer's advocate · Tampa Bay launch · Prepared 2026-09-13 for Manny (DIGIT2AI) and Ole
...
- **[GUESS]** a figure or fact I could not verify from here. Replace it with Ole's real numbers before anyone relies on it.
...
| 5 | Does Ole already run a CRM he will not leave? | The platform is the CRM; a one-way export to his existing CRM is added later if needed | Integration work in weeks 5 to 8 |
...
2. **A detected removal hides at once; a detected addition waits.** If monitoring sees an incentive shrink or disappear, it is pulled from new reports immediately, before Ole looks. If it sees a new or larger incentive, that waits for Ole. Mistakes therefore fall on the side of showing buyers less, never more.
...
| "Is this a good deal?" | The builder's salesperson | Ole, who represents the buyer and knows the sales reps |
...
**4. The local move-up family who already toured a model.** Lives in Hillsborough or Pasco, has visited two communities on their own and signed a guest card at one. The intake must catch this: representation may already be forfeited at that builder, and Ole needs to know before promising anything.
...
| Stage | What the buyer does | What the system does | What Ole does |
|---|---|---|---|
...
| Commission rebate to buyers | **Open decision** | Florida generally permits rebates to buyers when disclosed and allowed by the lender. Rebate brokerages compete on exactly this. A partial rebate could be a strong differentiator, and it cuts margin. Needs Ole's broker, the lender caps and an attorney. |

## Unit economics

**Every figure below is a [GUESS] to be replaced with Ole's closed-deal history.** They exist to show the shape of the model and which numbers matter.
...
Organic intake (community-name searches, referrals, Ole's past clients) costs close to nothing and is expected to be a large share within 6 months. [GUESS]
...
Not included: DIGIT2AI's build and maintenance time, Ole's time, the ad budget (counted in acquisition cost), and any licensed data feed.
...
- **The ceiling is Ole's capacity** [GUESS]: a solo new-construction agent with AI support might handle 50 to 70 closings a year. That is roughly $430k to $600k in net commission at base assumptions. Growth beyond that requires licensed associates or the licensing business.
...
2. **Ole's relationships and judgment, captured as structured knowledge.** Portals have traffic and builders have their own data. Neither has a 30-year agent sitting on the buyer's side of the table.

Honest weakness: Zillow and NewHomeSource win on traffic, and Incentiva cannot outspend them. The go-to-market has to lean on intent-specific search ("[builder] incentives Tampa"), Ole's network, relocation partners and MacDill.
...
```
detected ──(extraction ok)──▶ pending_verification ──(Ole confirms)──▶ verified
    │                                   │                                   │
    └──(low confidence/garbage)──▶ rejected        (Ole rejects)──▶ rejected │
                                                                            ▼
...
- **Freshness window** [DEFAULT]: verified with a published expiration = fresh until the earlier of `expires_on` or 21 days after `last_verified_at`. Verified with no published expiration = fresh for 10 days. A stale incentive drops out of new reports automatically and goes back into Ole's queue as `reconfirm`.
- **Asymmetric safety**: `removed` and `decrease` changes set the current version to `withdrawn` for new reports as soon as they are detected, then ask Ole to confirm. `new`, `increase` and `terms_changed` wait for Ole.
...
The extractor wrote `"up to"` as a cap, did not guess which homes qualify, and put the open questions in front of Ole. Until Ole answers, the payment scenario for this incentive says "amount confirmed up to $15,000; qualifying homes not confirmed" and is not ranked.
...
3. **Nothing sends without a gate.** Agents create drafts. The Compliance guard reviews them. Ole approves them, or they go out automatically under a template Ole approved in advance (see deliverable 7).
4. **Keyless still works.** With no model key, intake runs as a structured form, reports use templated narrative marked `generated_by: heuristic`, extraction queues sources for manual entry, and compliance runs its deterministic rules and holds everything for Ole.
...
| Advisor | `claude-sonnet-5` | Tone and judgment in Ole's voice |
...
```
 ...
 │ MONITOR    │──▶ Ole ──▶ incentive notes  ┌────────────┐
 └────────────┘  queue     versions         │ COMPLIANCE │── hold ──▶ Ole
        │                                   │   GUARD    │
        │ verified changes                  └─────┬──────┘
        ▼                                         │ pass
 ┌────────────┐  drafts ─────────────────────────▶│
...
| `handoff_to_human(reason)` | Notifies Ole; used for anything outside scope |
...
**Human in the loop.** Any `has_other_agent = yes_under_agreement` ends the conversation politely with no follow-up and no report. Ole sees these daily. Any buyer who mentions a disability accommodation, a legal dispute or an existing builder contract is handed to Ole.
...
- If they already visited or registered somewhere: record it. Tell them Ole will
...
- Invent anything. If a tool fails, say you will have Ole follow up, then call
  handoff_to_human.
...
| `agent_notes(scope_ids, visibility='advisor_may_use')` | Ole's notes for the Advisor |
...
**Human in the loop.** None at runtime; the report goes to the Advisor, then the Compliance guard, then Ole's approval queue (during the first 60 days, all reports; afterwards, holds only).
...
**Role.** Watches every source, detects change, extracts incentives from what changed, compares them with the current verified state, and turns real changes into verification cards for Ole. It never makes an incentive visible to buyers.

**Triggers.** Scheduler (per-source cadence, deliverable 5); inbound email to the ingest address; flyer or screenshot upload; a note from Ole tagged `unpublished_incentive`.
...
| `create_verification_card(version_id, questions[])` | Puts the card in Ole's queue with source excerpt, highlighted span and specific questions |
| `mark_source_health(source_id, status, reason)` | Blocked, failing or ok |
| `draft_rep_question(community_id, questions[])` | Drafts a short text or email Ole can send to the sales counselor |

**Human in the loop.** Every buyer-facing change requires Ole. The monitor may on its own: hide an incentive after detecting a removal or decrease, pause a failing source, and re-queue a stale incentive for reconfirmation.
...
**Role.** Writes the buyer-facing recommendation in Ole's voice, grounded only in the report's computed items and Ole's notes marked `advisor_may_use`. It encodes Ole's judgment: which incentive structure tends to be worth more for this buyer's situation, what to ask the sales counselor, and what to watch for.

**Knowledge base.** Ole's notes (typed, dictated as voice memos and transcribed, or forwarded) plus a **voice sample set**: 10 to 20 of Ole's real past emails and texts to buyers, with personal details removed. Retrieval pulls notes scoped to the report's builders and communities first, then market-level and general notes.
...
**Human in the loop.** The Advisor never speaks for Ole live. Every narrative goes through the Compliance guard; in the first 60 days, Ole approves every report before it sends.
...
```
You write the "Ole's take" section of a personalized new-construction report for a
homebuyer, in the voice of {agent_name}, a Florida real estate professional with more
than 30 years of experience working with new-home builders. You write as Ole, in the
...
- notes: Ole's own notes that may be used for advice.
- voice_samples: examples of how Ole writes.
...
| An incentive shown in a report the buyer received is withdrawn or decreased (after Ole confirms) | **Correction**, sent even without a marketing reason: they were told something that is no longer true |
| Registration expires in 7 days | Draft to Ole: renew or let lapse |
| Incentive `close_by` or `contract_by` date is within 21 days for a buyer under contract or touring | Deadline reminder to the buyer and to Ole |
...
**Human in the loop.** All messages are Ole-approved in batches until a template graduates. A template graduates when Ole has approved 20 sends from it with no edits. Graduated templates auto-send only if every fact in them is verified and fresh and the Compliance guard passes the rendered message.
...
- For corrections: say plainly what changed and when, and that Ole can walk them
...
**Role.** The last gate before anything reaches a buyer or the public: reports, messages, landing and community pages, and Ole's knowledge notes (before the Advisor may use them). **Fails closed**: if it errors or times out, the item is held.
...
1. **Deterministic rules (code, always run, keyless).**
   ...
   - Lending advertising triggers: rate, payment, down payment, number of payments or finance charge in a **marketing** message (not a personalized report) → hold for Ole [LAWYER; see deliverable 8].
...
**Human in the loop.** Every `hold` goes to Ole with the exact sentence and the rule it tripped. Ole can edit and release, or override with a written reason (logged). `block` (a fair-housing hard term, an unverified incentive, no consent) cannot be overridden from the dashboard; the content must change.
...
```
 BUYERS                          OLE (phone-first PWA)              BUILDER SOURCES
...
| Calendar | Google Calendar OAuth on Ole's calendar; booking slots computed from free/busy | Ole already lives in a calendar; no second one | Calendly embed |
| CRM | The platform itself (buyer pipeline + timeline); CSV/webhook export to Ole's existing CRM if he has one | One source of truth for consent and agreements | Sync to Follow Up Boss, kvCORE or similar |
...
| 5 | Sales counselor texts and calls | Ole types or dictates a note; the monitor structures it into a verification card | Unpublished incentives live here |
| 6 | Flyers and screenshots from model visits | Upload from phone → OCR → extraction | Common in sales offices |
| 7 | Broker portals (logged-in builder agent sites) | **Manual only**: Ole views, screenshots or copies; no automated login | Portal terms usually prohibit automated access and credential sharing |
...
- **Accuracy test set.** Keep 100 labelled snapshots (Ole-verified) as a regression set; any prompt or model change must hold field-level accuracy on it before it ships.

## Cadence [DEFAULT]

| Source kind | Normal | Last 10 days of a calendar quarter | Notes |
|---|---|---|---|
| Promotions pages | Daily, 05:30 ET | Twice daily (05:30, 13:30) | Builders commonly push closings at quarter-end [GUESS; confirm with Ole] |
...
| Reconfirmation queue build | Daily, 06:30 ET | Daily | Ready before Ole's morning review |
...
- A 403, 429 or bot-challenge page twice in a row → source `blocked`, fetching stops, Ole is notified.
...
1. **Email ingest.** Make sure Ole is on that division's broker email list and the forwarding rule is catching it.
2. **Ask.** Ole asks the division's broker relations contact for permission or a feed. Record `automated_access = permission_granted` with the date and who granted it.
...
{{withheld communities: name, builder, city, why it fits}}. Ole is confirming their
...
If you would like Ole to represent you at no cost to you, book a call first and he will
register you with each builder you want to see.
[Book a call with Ole]   [Ask a question]
...
AÚN EN CONFIRMACIÓN
{{comunidades retenidas: nombre, constructora, ciudad, por qué encaja}}. Ole está
...
de su primera visita. Si desea que Ole lo represente, sin costo para usted, reserve una
llamada primero y él lo registrará con cada constructora que quiera conocer.
[Reservar una llamada con Ole]   [Hacer una pregunta]
...
# 7. Ole's admin dashboard

**Principles.** Phone-first installable app (Ole is in model homes and trucks, not at a desk). The home screen is a work queue with a time estimate, not a chart. Every card has one primary action. Anything the AI proposed shows its source next to it.
...
**5. Registration packet**: pre-filled buyer name, contact, agent name, license, brokerage, agreement signed date; the builder's registration method and portal link; a drafted email to the community sales counselor. Ole submits, then taps "Submitted" and later "Confirmed" with the reference.
...
**9. Knowledge**: add a note by typing or voice memo; scope picker (builder, division, community, plan, sales rep, market); category; visibility (`internal_only` default); compliance status. Search by scope and text. Notes marked `avoid` require a stated objective reason (build quality, warranty history, HOA litigation, construction delays) and are `internal_only` unless Ole changes them and compliance clears them.
...
Owner key: **O** = Ole or his broker · **M** = Manny/DIGIT2AI · **A** = attorney · **S** = system (enforced in code, tested in SIT).

## Florida licensing and brokerage

- [ ] **A/O** Confirm Ole's license status (broker or sales associate), active and in good standing, and that his broker approves operating Incentiva, its advertising and its lead handling.
...
- [ ] **S** Agreement signed **before** any registration and before an agent-accompanied tour (practice changes following the 2024 NAR settlement require a written agreement before touring, including new construction; confirm with Ole's broker whether it binds him through his board or MLS).
...
- [ ] **S** Lexicon checks (EN/ES) on reports, messages, pages and Ole's notes; model review for implied steering.
...
- [ ] **O** Ole's "communities to avoid" notes carry objective, documented reasons only.
...
- [ ] **S** Removals and decreases hide immediately; buyers who received a now-incorrect incentive get a correction after Ole confirms.
...
- [ ] **O** Ole does not take loan applications or negotiate loan terms (that requires a mortgage loan originator license); estimates are labelled as comparisons, not offers.
...
- [ ] **S** The chat and WhatsApp assistant identifies itself as an automated assistant working for Ole's brokerage, in the first message.
- [ ] **S** Reports say that the narrative was drafted with AI assistance, and whether Ole personally reviewed that report or it was released under his approved review rules.
...
| Week | Dates | Build | Ole supplies | Exit test |
|---|---|---|---|---|
...
| Week | Dates | Focus | Ole supplies | Target by end of week [GUESS; adjust after week 7] |
|---|---|---|---|---|
...
| **6** Private soft launch | Oct 19 – 25 | 10–20 buyers from Ole's network and past-client referrals; Ole approves every report and message; fix what they trip over | Invites to real buyers; feedback after each consult | 10 intakes, 5 consults, 2 agreements signed |
...
| 8 | Does Ole have a CRM he will keep using? | No; platform is the CRM | Week 4 |
...
|---|---|---|---|
| **Compensation structure found non-compliant** (unlicensed person sharing commission) | Medium | Severe: fines, license discipline for Ole, unwinding the venture | Attorney opinion before any revenue agreement; flat fee default |
| **Builders refuse co-broke for online-originated buyers** or treat a buyer who first contacted them online as unrepresented | Medium | High: the revenue model depends on it | Warn buyers before visits; register before first contact; record each builder's written policy; Ole's relationships |
| **Builders object to public comparison of their incentives**, straining Ole's relationships | Medium | High | Keep builder values in private reports; no creative content; Ole briefs divisions early; facts only, always with the builder's own conditions |
...
| **Ole is a single point of failure** (capacity, illness, retirement) | Medium | Severe | Knowledge capture from day one; a licensed associate trained by week 13; the verification process documented so a second agent can run it |
...
- Every market figure (prices, co-broke, conversion, ad costs, community counts) is an estimate until replaced with Ole's data and week 9 measurements.
- No legal conclusions: every [LAWYER] item needs a Florida real estate attorney's written answer.
- Insurance and property-tax assumptions in payment estimates need a local source Ole trusts, refreshed at least quarterly.
