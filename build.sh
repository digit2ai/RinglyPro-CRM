...
echo ""
echo "================================================"
echo "Building ImagingMind Dashboard"
echo "================================================"

if [ -d "verticals/msk_intelligence/frontend" ]; then
    cd verticals/msk_intelligence/frontend
    echo "ImagingMind Dashboard directory: $(pwd)"

    if [ -d "dist" ] && [ -f "dist/index.html" ]; then
        echo "✅ ImagingMind dist folder already exists (pre-built), skipping build"
        ls -lh dist/
        cd ../../..
    else
        echo "📦 Installing ImagingMind dashboard dependencies..."
        NODE_ENV=development npm ci --include=dev || NODE_ENV=development npm install --include=dev

        echo ""
        echo "🔨 Building ImagingMind dashboard with Vite..."
        set +e
        NODE_ENV=production npm run build 2>&1
        BUILD_EXIT_CODE=$?
        set -e
        ...
        if [ $BUILD_EXIT_CODE -ne 0 ]; then
          echo "⚠️ ImagingMind build failed with exit code $BUILD_EXIT_CODE"
        fi

        if [ -d "dist" ]; then
            echo "✅ ImagingMind dist folder created!"
            ls -lh dist/
        else
            echo "⚠️ ImagingMind dist folder not created"
        fi

        cd ../../..
    fi
else
    echo "⚠️ ImagingMind dashboard directory not found, skipping..."
fi
