...
- **SIT:** `node client-builds/ai-agent-prompt-builder-for-data-writing/sit.js` → **126/126**, zero external keys — it unsets `ANTHROPIC_API_KEY` before requiring the app and says so, so the suite is free and offline and the keyless fallback is the one under test. Consequence: the model path is only verifiable against production (`GET /health` → `composer`).
