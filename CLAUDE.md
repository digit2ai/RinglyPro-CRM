...
**PAGE ACTIONS: the orb can act on its page, never on the server.** A pack may declare `pageActions: [{ name, description, input_schema, sanitize }]` (first user: `buyersline` → `fill_intake_form`). The route offers them to the model as tools but NEVER executes them: `sanitize()` keeps only allow-listed, valid values, the tool result tells the model the page was filled but NOT sent, and the clean values come back in the response as `actions`. `voice-orb.js` fires a `d2orb:action` window event per action and the host page decides what to do. **Anything that must stay the user's own act (consent, submission) is simply absent from the schema**, so no model output can reach it. Packs without `pageActions` behave exactly as before.
...
**Martha fills the intake form by voice.** The landing's voice orb (persona `buyersline`, display name **Martha**, Edge voice Ava/Dalia) calls `fill_intake_form` with only what the buyer said; the landing listens for `d2orb:action`, fills and outlines the fields (`.ava-filled`), and scrolls to the form. It NEVER ticks a consent box or submits; the schema has no field for either and SIT greps the listener. Verified in production 2026-09-14: English and Spanish fills in one turn. Prompting alone did not stop the model asking about optional fields before filling; the persona now says optional fields never block. Highlight colors use `rgba`, not `color-mix()`, which older browsers drop silently.
...
**SIT:** `node verticals/incentiva/sit.js` → **94/94**, zero external keys (removes `ANTHROPIC_API_KEY`, geocoding off). DB sections run as tenants 990913/990914 and delete their rows. Not covered: the model extraction/narrative paths, live geocoding, live builder fetches.
