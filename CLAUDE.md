...
**Endpoints:** `GET /health` · pages `/` `/r/:token` `/login` `/admin` · public `GET /api/v1/public/config` · `POST /buying-power` · `POST /intake` · `GET /reports/:token` · `POST /reports/:token/consult-request` · auth `POST /api/v1/auth/login|logout`, `GET /me` · agent `GET /today` · `GET /verifications`, `POST /verifications/:id/confirm|reject`, `POST /incentives/:id/reconfirm`, `GET|POST /incentives` · `GET|POST /builders` · `GET|POST /communities`, `PATCH /communities/:id`, `POST /communities/:id/fees|homes` · `POST /sources`, `POST /sources/:id/fetch` · `POST /ingest/text` · `GET /reports`, `GET /reports/:id/preview` (read-only, the buyer view of a held report; the console opens `/r/:token?preview=:id`, which shows an Agent preview banner and no consult form), `POST /reports/:id/approve|hold` · `GET /compliance`, `POST /compliance/:id/release` · `GET /buyers`, `GET /buyers/:id` · `POST /appointments/:id/held` · `GET /billing` · `GET|PUT /settings/market` · `POST /demo/seed|reset`. Debug: `/debug/buyersline-error`. Voice orb persona `buyersline` in `src/config/voice-agents.js`.

**Martha drives the intake form to a sent report.** The landing voice orb (persona `buyersline`, display name **Martha**, Edge voice Ava/Dalia) has two page actions. `fill_intake_form` fills and outlines (`.ava-filled`) only what the buyer said. **She asked every question twice before 2026-09-14** because the orb history keeps only spoken text, not tool calls, and the page text carries no form values; the landing now exposes `D2AIVoiceOrbLiveContext` (INTAKE FORM STATUS: filled values, required still missing, optional, contact-box states, send state) and the persona must read it first, never re-ask a filled field, and ask for the next missing required field in a fixed order. `submit_intake_form` has an empty schema and three locks: the server honors it only when the buyer's latest words are a clear yes with no no/wait/not yet (`blConfirmSubmit`), the page refuses while a required field is empty (and highlights it), and a 5-second countdown bar with Cancel runs before `requestSubmit`. **Consent boxes are never written by the page script** and SIT checks every line of it that mentions consent. Highlight colors use `rgba`, not `color-mix()`, which older browsers drop silently.
...
**SIT:** `node verticals/incentiva/sit.js` → **100/100**, zero external keys (removes `ANTHROPIC_API_KEY`, geocoding off). DB sections run as tenants 990913/990914 and delete their rows. Not covered: the model extraction/narrative paths, live geocoding, live builder fetches.
