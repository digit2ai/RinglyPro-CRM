...
**Voice.** The orb's Ana fills chat answers through `BLChat.applyVoice` (area, price, monthly, down, timing, financing, name, email, phone, agent) and reads `ANA CHAT STATUS` via `D2AIVoiceOrbLiveContext`. She cannot pick communities or tick consent (SIT greps both files); `submit_intake_form` still needs a spoken clear yes, the last step with nothing missing, and a 5-second cancelable countdown.
...
**SIT:** `node verticals/incentiva/sit.js` → **116/116**, zero external keys (removes `ANTHROPIC_API_KEY`, geocoding off). DB sections run as tenants 990913/990914 and delete their rows. Not covered: the model extraction/narrative paths, live geocoding, live builder fetches.
