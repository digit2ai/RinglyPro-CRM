...
- **T4.2 Portuguese (PT-BR)**: third lang pill (EN / ES / PT). Body classes `lang-en` / `lang-es` / `lang-pt` drive CSS show/hide. Missing PT translations get `.pt-fallback` on the i18n-en sibling at lang-pt activation so visitors see EN content (graceful degradation). I18N dictionary block `pt` has full keyboard-demo strings. Hero copy, orb caption, sticky CTA, approval banner, brain-narrating pill, result-panel buttons, SR labels all explicitly translated. New env var: **`ELEVENLABS_CONVAI_PARTNERSHIP_PT`** — when unset, `/partnership-orb-config?lang=pt` returns the EN agent (Adam, multilingual) with `fell_back_to: 'en'`.
