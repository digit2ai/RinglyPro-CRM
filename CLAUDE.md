...
**Portada de `maramed.app`:** `public/maramed-landing.html` (manejador de host en `src/app.js`; `www` hace 301 al apex). Presenta la plataforma completa (cinco módulos, privacidad, estado honesto; la sección de arquitectura se retiró a pedido del dueño y vive solo en el mazo) con las pantallas reconstruidas en HTML y **datos simulados**: ningún nombre, cédula ni fotografía de paciente de las capturas reales va a una página pública. Los CTA llevan a `maramed.vercel.app` (la aplicación de ISTC); `maramed.app/maramed/` es la demo técnica rPPG de Digit2AI: **la portada no la enlaza** (retirada a pedido del dueño, porque la aplicación real ya está en vivo) y nunca debe presentarse como el motor de VitalScan. La narradora de la portada es **Mara · Voz AI de MaraMed** (voz Edge es-MX-DaliaNeural por `/api/tts/edge` del mismo origen), ES/EN, sin emojis. Mismas cifras admisibles que el mazo.
...
**SIT:** `node verticals/incentiva/sit.js` → **144/144**, zero external keys (removes `ANTHROPIC_API_KEY`, geocoding off). DB sections run as tenants 990913/990914 and delete their rows. Not covered: the model extraction/narrative paths, live geocoding, live builder fetches.
...
- `INCENTIVA_SITE_GATE` (on unless `off`) — puts the whole site behind the architecture sign-in until launch.
