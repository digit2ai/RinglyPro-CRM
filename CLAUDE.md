...
**The lit sign** (`public/jobmd-sign.jpg`, hero; `og-image.jpg`, social card) was asked for as "the main image from surgicalmind.app". **That file is the SurgicalMind wordmark**, so shipping it verbatim would have put another product's brand across this page — the treatment was rebuilt with JobMD's own name and palette instead ("Job" etched into the wall, "MD" lit in the JobMD cyan, mirroring SurgicalMind's own grey/blue split). It is rendered by Chrome via Puppeteer, not by sips: **sips ignores `fill` on `<tspan>`**, which silently rendered the whole wordmark black, and its `feDropShadow` support is unreliable. JPEG not PNG — smooth gradients go 583KB to 92KB. SIT fails if `surgicalmind` appears in the page or if any asset is hotlinked from another host's CDN.
