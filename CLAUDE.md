...
**SIT:** `node verticals/jobup/sit.js` → **324/324**, zero external keys.
...
- `JOBUP_SUBS_ADMIN_EMAIL` (default `admin@jobup.dev`) / `JOBUP_SUBS_ADMIN_PASSWORD` (default `Palindrome@7`) — the subscribers/billing console at `/subscribers-admin`. **Unlike `/admin`, this one ships with a working default so it is usable out of the box, which means the credential is in the source of a public repo. SET `JOBUP_SUBS_ADMIN_PASSWORD` on Render.** Until you do, `GET /subscribers-admin/api/health` reports `using_default_password:true` and the console renders a warning banner — the exposure is surfaced, not buried. Changing the email env var revokes live sessions immediately (the token is re-checked against current config on every request).
