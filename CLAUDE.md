...
**Versión MP4 para WhatsApp:** `node scripts/render-maramed-video.js [--mb=9.6] [--out=...]` → **8,56 MB · 8:00 · 1280x720 · 12 fps** (ffmpeg es dependencia **de build**, no del servidor; el MP4 no se versiona, se regenera). Tres decisiones que no conviene deshacer: (1) **la secuencia de fotogramas se arma a mano, no con el demuxer `concat`** — con directivas `duration` ffmpeg deriva su propio reloj y el video salía 717 s contra 676 s de voz, con las láminas separándose de lo que Dalia iba diciendo; ahora cada lámina ocupa `round(duración_mp3 × 12)` fotogramas y el desfase total queda en 56 ms. (2) **El bitrate se despeja del techo, no se elige**: el límite de WhatsApp es de tamaño, se reparte el presupuesto entre audio y video y se codifica en dos pasadas; subir la resolución con el mismo presupuesto empeora la imagen — a ~96 kbps, 720p se ve mejor que 1440p. (3) **El subtítulo se achica hasta que entra** (de 13,5 px hacia abajo): en video no hay barra de desplazamiento, ocho de las doce narraciones se cortaban a mitad de frase, y es justo el texto que lee quien mira WhatsApp en silencio. **El guion se apretó un 30% (10.201 → 7.181 caracteres) sin perder un solo dato ni una sola advertencia**: la primera versión duraba 11:16 y se cortaba al enviarla por WhatsApp. Como el peso es duración × bitrate, acortar el relato bajó el archivo de 11,15 a 8,56 MB **y subió** el bitrate de 96k a 120k, así que además se ve mejor. No recortar la lámina once ni el recuadro de limitación para ahorrar segundos: son la razón de ser del mazo. **El techo se comprueba, no se pide** — el script codifica, mide y corrige el bitrate hasta caer en la banda 88-100% del techo (con `-maxrate`/`-bufsize`), y **falla** si excede el techo o si video y voz difieren en más de un segundo.

**Corte VERTICAL para compartir (WhatsApp):** `public/maramed-vertical.html` + `public/maramed-audio-vertical/v1..v9.mp3` + `scripts/generate-maramed-vertical-audio.js` + `scripts/render-maramed-vertical-video.js` → **8,16 MB · 56 s · 1080x1920 · 25 fps**. **Es otro video, no un recorte del mazo**: once minutos en horizontal quedan como una franja ilegible en un teléfono en vertical, y nadie reenvía once minutos. Aquí las pantallas del producto —que ya son verticales— se escalan ×2,42 y ocupan el alto completo, con subtítulo quemado a 44 px porque en WhatsApp se ve en silencio. **Reutiliza el marcado de los teléfonos del mazo largo** (se extrae de `maramed-presentacion.html` en tiempo de construcción), así que las pantallas no se pueden ir separando entre las dos piezas. Nueve escenas: gancho, biometría, triaje hablado, nivel Manchester, escaneo, cinco signos, informe, **el límite del sistema** (no se omite por ser un corte comercial) y cierre ISTC × Digit2AI. **El techo de tamaño se comprueba, no se pide**: con sólo nueve imágenes distintas el control de tasa de x264 se pasó un 24% en la primera prueba, así que el script codifica, mide y corrige el bitrate hasta que entra (más `-maxrate`/`-bufsize`), y **falla** si excede el techo o si video y voz difieren en más de medio segundo. Cada escena lleva 0,45 s de silencio al final: sin esa cola el corte cae encima de la última sílaba.
