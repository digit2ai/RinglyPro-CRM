...
**Voice.** The orb's Martha fills chat answers through `BLChat.applyVoice` (area, price, monthly, down, timing, financing, name, email, phone, agent) and reads `MARTHA CHAT STATUS` via `D2AIVoiceOrbLiveContext`. She cannot pick communities or tick consent (SIT greps both files); `submit_intake_form` still needs a spoken clear yes, the last step with nothing missing, and a 5-second cancelable countdown.
