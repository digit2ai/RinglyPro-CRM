// kancho-ai/services/kancho-voice-call-service.js
// Twilio + ElevenLabs outbound calling service for Kancho AI

const twilio = require('twilio');

class KanchoVoiceCallService {
  constructor() {
  ...
  initializeTwilio() {
    ...
    if (accountSid && authToken && phoneNumber) {
      try {
        ...
        console.log('[KanchoVoice] Twilio initialized successfully');
      } catch (error) {
        console.error('[KanchoVoice] Failed to initialize Twilio:', error.message);
      }
    } else {
      console.warn('[KanchoVoice] Twilio credentials not configured - outbound calling disabled');
    }
  }
   ...
   * @param {string} options.agent - Agent type ('kancho' or 'maestro')
  ...
  async initiateOutboundCall(options) {
    const { school_id, phone, agent = 'kancho', call_type, lead_id, student_id, context } = options;
    ...
    const twimlUrl = new URL(`${this.webhookBaseUrl}/kanchoai/api/v1/voice/twiml`);
    ...
    const statusCallbackUrl = `${this.webhookBaseUrl}/kanchoai/api/v1/voice/twilio-status`;
    ...
    try {
      const call = await this.twilioClient.calls.create({
        ...
        record: true,
        recordingStatusCallback: `${this.webhookBaseUrl}/kanchoai/api/v1/voice/recording-status`,
        recordingStatusCallbackMethod: 'POST'
      });

      console.log(`[KanchoVoice] Outbound call initiated: ${call.sid} to ${formattedPhone}`);
      ...
      console.error('[KanchoVoice] Failed to initiate call:', error.message);
      throw error;
    }
  }
  ...
  async getCallStatus(callSid) {
    ...
    try {
      ...
      console.error('[KanchoVoice] Failed to get call status:', error.message);
      throw error;
    }
  }
  ...
  async endCall(callSid) {
    ...
    try {
      ...
      console.log(`[KanchoVoice] Call ended: ${callSid}`);
      return { success: true, status: call.status };
    } catch (error) {
      console.error('[KanchoVoice] Failed to end call:', error.message);
      throw error;
    }
  }
  ...
  async getRecordingUrl(callSid) {
    ...
    try {
      ...
      console.error('[KanchoVoice] Failed to get recording:', error.message);
      return null;
    }
  }

  /**
   * Map Twilio call status to KanchoAiCall status
   * @param {string} twilioStatus - Twilio status string
   * @returns {string} - KanchoAiCall status
   */
...
module.exports = new KanchoVoiceCallService();
